...
class Client {
	...
	constructor (window, server) {
		this._rpc = RPC(window, server, '*', this._methods())
	}

	getStorage() {
			return RandomAccessRPC.client(rpc)
	}
	...
	createArchive(options, key, callback) {
		rpc.call('selectArchive', options, key, callback)
	}
...
class Server {
	...
	_methods() {
		return Object.assign(RandomAccessRPC.server(this.storage), {
			...
			addArchive: (options, key, callback) => {
				this.addArchive(options, key, callback)
			}
		})
	}
}
